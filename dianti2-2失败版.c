...
int main()
{
    ...
    do {
        for ( i= 0;  i< human; i++) {//判断乘客是否在当时按电梯
            if (timenow >= time[i]) {
                if (p1 != 4 && (d[i]==w1 || w1==0) && (begin[i]-floornow1)*w1>=0) aim1[i]=begin[i];
                if (p2 != 4 && (d[i]==w2 || w2==0) && (begin[i]-floornow2)*w2>=0) aim2[i]=begin[i];
                //printf("%d",(p2 != 4 && (d[i]==w2 || w2==0) && (begin[i]-floornow2)*w2>=0));
                //if(w1==0) w1=d[i];
                //if(w2==0) w2=d[i];
        ...
        for (i = 0,judge = 0; i < human; i++){//判断响应哪个电梯及判断方向
            if (min > d1[i] && p1 != 4 && (d[i]==w1 || w1==0) && (begin[i]-floornow1)*w1>=0){
            min = d1[i];
            judge=1;
            }
            if (min > d2[i] && p2 != 4 && (d[i]==w2 || w2==0) && (begin[i]-floornow2)*w2>=0){
            min = d2[i];
            judge=2;
            }
        }
